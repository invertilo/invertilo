...
* **🛡️ Simulación Ofensiva para Desarrollo Defensivo:** Recreación y desarrollo de exploits conceptuales (PoC) para documentar fallos de integridad y proporcionar firmas e indicadores de compromiso (IoCs) a desarrolladores de anticheats locales para que mitiguen estas brechas de seguridad.
...
A continuación se realiza un desglose técnico de los proyectos principales de la suite **Fernet**:
...
* **📡 Algoritmo de Predicción de Latencia (Ping Prediction):**
  El módulo de apuntado intercepta y estima dinámicamente la latencia de red midiendo los tiempos de ida y vuelta. Utiliza esta estimación para calcular la posición futura del objetivo físico mediante la velocidad física detectada en su componente `AssemblyLinearVelocity`.
* **📈 Suavizado de Trayectoria Bilateral (Exponential Smoothing):**
  Divide los cálculos de interpolación angular para los ejes horizontal (Yaw) y vertical (Pitch) de manera independiente. Esto evita movimientos robóticos y permite una simulación de entrada de mouse orgánica.
...
* **🖱️ Cursor Personalizado e Interactivo:**
...
* **☄️ Bullet Tracers & Movement Trails:** Dibuja líneas tridimensionales que conectan el cañón del arma del oponente con el punto final del proyectil, y genera estelas que marcan la ruta de movimiento previa de los objetivos.
* **☀️ Ambient Customizer & Skybox Changer:** Modificación de las propiedades de renderizado nativas del motor en memoria. Permite la eliminación total de niebla (`fog_remove`), la configuración de la distancia e intensidad del fog y la sobreescritura del skybox para aplicar un fondo negro/estrellado estético.
...
**Fernet FiveM** es un software externo de alto rendimiento especializado en la interacción de memoria con el motor de juego modificado de GTA V (FiveM). Su arquitectura se fundamenta en un motor de lectura asíncrona optimizado que recopila y proyecta información en un overlay invisible de alto rendimiento.
...
* **⏱️ Triggerbot Inteligente:** Automatiza el disparo del arma actual al detectar que la retícula de apuntado pasa por encima de las coordenadas del modelo de colisión de un oponente válido. Incorpora retardos configurables en milisegundos para emular tiempos de respuesta humanos.

#### 👁️ ESP y Proyección de Coordenadas (World-to-Screen)
* **🖥️ Módulo World-to-Screen:** Traducción dinámica de coordenadas 3D (la posición real de los jugadores y entidades en el mapa) a coordenadas 2D (los píxeles en tu pantalla) mediante la lectura externa de la matriz de vista de la cámara. Esto permite que el overlay dibuje el ESP de forma alineada sin importar la resolución o el campo de visión.
...
* **⚙️ Handling Extractor & Patching:** Mapea la dirección de memoria de la clase handling del vehículo actual para modificar variables físicas en tiempo real. Permite alterar la masa física, la fuerza de tracción lateral, el factor de fricción aerodinámica y la aceleración instantánea.
...
Es la aplicación y cargador principal diseñado para que los compradores que adquieren una suscripción a los softwares descritos puedan ejecutar, administrar y utilizar las herramientas de asistencia adquiridas de forma segura y automatizada. Desarrollada en **C++20**, es una interfaz gráfica de alto nivel que protege el software contra manipulaciones y asegura la integridad de la ejecución en la máquina del cliente bajo un modelo de licenciamiento controlado.
...
* **[Fernet Loader](https://github.com/invertilo/fernet):** Cargador y gestor de licencias desarrollado en C++ y Dear ImGui que permite a los compradores de una suscripción ejecutar de forma segura y automatizada los softwares de ventajas adquiridos, implementando técnicas de evasión de firmas y autodefensa avanzada en memoria.
